/* tslint:disable */
/* eslint-disable */
/**
 * ACBr API
 * API para automação comercial e documentos fiscais.    **Autenticação**  Esta API usa OAuth 2.0 no fluxo `client_credentials`.    **Ambientes disponíveis**  - Produção: `https://prod.acbr.api.br`  - Homologação: `https://hom.acbr.api.br`  - Autenticação para ambos os ambientes: `https://auth.acbr.api.br/realms/ACBrAPI/protocol/openid-connect/token`    **Endpoint para obter o token**  - `POST https://auth.acbr.api.br/realms/ACBrAPI/protocol/openid-connect/token`    **Content-Type**  - `application/x-www-form-urlencoded`    **Parâmetros do corpo**  - `grant_type=client_credentials`  - `client_id=SEU_CLIENT_ID`  - `client_secret=SEU_CLIENT_SECRET`  - `scope=empresa nfe nfse`    **Exemplo de corpo**  - `grant_type=client_credentials&client_id=SEU_CLIENT_ID&client_secret=SEU_CLIENT_SECRET&scope=empresa%20nfe`    **Uso do token nas requisições**  - `Authorization: Bearer SEU_ACCESS_TOKEN`
 *
 * The version of the OpenAPI document: 3.1.7
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
/**
 * Identificação do Marketplace (uso exclusivo Marketplace).
 * @export
 * @interface DceSefazMarketplace
 */
export interface DceSefazMarketplace {
    /**
     * Número do CNPJ do Marketplace.
     * @type {string}
     * @memberof DceSefazMarketplace
     */
    CNPJ: string | null;
    /**
     * Razão Social ou Nome do Marketplace.
     * @type {string}
     * @memberof DceSefazMarketplace
     */
    xNome: string | null;
    /**
     * Endereço ou site do Marketplace.
     * @type {string}
     * @memberof DceSefazMarketplace
     */
    Site: string | null;
}

/**
 * Check if a given object implements the DceSefazMarketplace interface.
 */
export function instanceOfDceSefazMarketplace(value: object): boolean {
    let isInstance = true;
    isInstance = isInstance && "CNPJ" in value;
    isInstance = isInstance && "xNome" in value;
    isInstance = isInstance && "Site" in value;

    return isInstance;
}

export function DceSefazMarketplaceFromJSON(json: any): DceSefazMarketplace {
    return DceSefazMarketplaceFromJSONTyped(json, false);
}

export function DceSefazMarketplaceFromJSONTyped(json: any, ignoreDiscriminator: boolean): DceSefazMarketplace {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'CNPJ': json['CNPJ'],
        'xNome': json['xNome'],
        'Site': json['Site'],
    };
}

export function DceSefazMarketplaceToJSON(value?: DceSefazMarketplace | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'CNPJ': value.CNPJ,
        'xNome': value.xNome,
        'Site': value.Site,
    };
}

