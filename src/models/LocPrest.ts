/* tslint:disable */
/* eslint-disable */
/**
 * ACBr API
 * API para automação comercial e documentos fiscais.    **Autenticação**  Esta API usa OAuth 2.0 no fluxo `client_credentials`.    **Ambientes disponíveis**  - Produção: `https://prod.acbr.api.br`  - Homologação: `https://hom.acbr.api.br`  - Autenticação para ambos os ambientes: `https://auth.acbr.api.br/realms/ACBrAPI/protocol/openid-connect/token`    **Endpoint para obter o token**  - `POST https://auth.acbr.api.br/realms/ACBrAPI/protocol/openid-connect/token`    **Content-Type**  - `application/x-www-form-urlencoded`    **Parâmetros do corpo**  - `grant_type=client_credentials`  - `client_id=SEU_CLIENT_ID`  - `client_secret=SEU_CLIENT_SECRET`  - `scope=empresa nfe nfse`    **Exemplo de corpo**  - `grant_type=client_credentials&client_id=SEU_CLIENT_ID&client_secret=SEU_CLIENT_SECRET&scope=empresa%20nfe`    **Uso do token nas requisições**  - `Authorization: Bearer SEU_ACCESS_TOKEN`
 *
 * The version of the OpenAPI document: 3.1.7
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
/**
 * Grupo de informações relativas ao local da prestação do serviço.
 * @export
 * @interface LocPrest
 */
export interface LocPrest {
    /**
     * Código do município onde o serviço foi prestado (tabela do IBGE).
     * 
     * Caso não seja informado, será considerado o município do Prestador do serviço.
     * @type {string}
     * @memberof LocPrest
     */
    cLocPrestacao?: string | null;
    /**
     * Código do país onde o serviço foi prestado (Tabela de Países ISO).
     * @type {string}
     * @memberof LocPrest
     */
    cPaisPrestacao?: string | null;
}

/**
 * Check if a given object implements the LocPrest interface.
 */
export function instanceOfLocPrest(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function LocPrestFromJSON(json: any): LocPrest {
    return LocPrestFromJSONTyped(json, false);
}

export function LocPrestFromJSONTyped(json: any, ignoreDiscriminator: boolean): LocPrest {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'cLocPrestacao': !exists(json, 'cLocPrestacao') ? undefined : json['cLocPrestacao'],
        'cPaisPrestacao': !exists(json, 'cPaisPrestacao') ? undefined : json['cPaisPrestacao'],
    };
}

export function LocPrestToJSON(value?: LocPrest | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'cLocPrestacao': value.cLocPrestacao,
        'cPaisPrestacao': value.cPaisPrestacao,
    };
}

