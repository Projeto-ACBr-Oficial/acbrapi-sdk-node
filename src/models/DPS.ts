/* tslint:disable */
/* eslint-disable */
/**
 * ACBr API
 * API para automação comercial e documentos fiscais.    **Autenticação**  Esta API usa OAuth 2.0 no fluxo `client_credentials`.    **Ambientes disponíveis**  - Produção: `https://prod.acbr.api.br`  - Homologação: `https://hom.acbr.api.br`  - Autenticação para ambos os ambientes: `https://auth.acbr.api.br/realms/ACBrAPI/protocol/openid-connect/token`    **Endpoint para obter o token**  - `POST https://auth.acbr.api.br/realms/ACBrAPI/protocol/openid-connect/token`    **Content-Type**  - `application/x-www-form-urlencoded`    **Parâmetros do corpo**  - `grant_type=client_credentials`  - `client_id=SEU_CLIENT_ID`  - `client_secret=SEU_CLIENT_SECRET`  - `scope=empresa nfe nfse`    **Exemplo de corpo**  - `grant_type=client_credentials&client_id=SEU_CLIENT_ID&client_secret=SEU_CLIENT_SECRET&scope=empresa%20nfe`    **Uso do token nas requisições**  - `Authorization: Bearer SEU_ACCESS_TOKEN`
 *
 * The version of the OpenAPI document: 3.1.7
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
/**
 * Grupo de informações da DPS relativas ao serviço prestado.
 * @export
 * @interface DPS
 */
export interface DPS {
    /**
     * Série do DPS.
     * @type {string}
     * @memberof DPS
     */
    serie?: string | null;
    /**
     * Número do DPS.
     * @type {string}
     * @memberof DPS
     */
    nDPS?: string | null;
}

/**
 * Check if a given object implements the DPS interface.
 */
export function instanceOfDPS(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function DPSFromJSON(json: any): DPS {
    return DPSFromJSONTyped(json, false);
}

export function DPSFromJSONTyped(json: any, ignoreDiscriminator: boolean): DPS {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'serie': !exists(json, 'serie') ? undefined : json['serie'],
        'nDPS': !exists(json, 'nDPS') ? undefined : json['nDPS'],
    };
}

export function DPSToJSON(value?: DPS | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'serie': value.serie,
        'nDPS': value.nDPS,
    };
}

