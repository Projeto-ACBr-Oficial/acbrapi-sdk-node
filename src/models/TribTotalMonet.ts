/* tslint:disable */
/* eslint-disable */
/**
 * ACBr API
 * API para automação comercial e documentos fiscais.    **Autenticação**  Esta API usa OAuth 2.0 no fluxo `client_credentials`.    **Ambientes disponíveis**  - Produção: `https://prod.acbr.api.br`  - Homologação: `https://hom.acbr.api.br`  - Autenticação para ambos os ambientes: `https://auth.acbr.api.br/realms/ACBrAPI/protocol/openid-connect/token`    **Endpoint para obter o token**  - `POST https://auth.acbr.api.br/realms/ACBrAPI/protocol/openid-connect/token`    **Content-Type**  - `application/x-www-form-urlencoded`    **Parâmetros do corpo**  - `grant_type=client_credentials`  - `client_id=SEU_CLIENT_ID`  - `client_secret=SEU_CLIENT_SECRET`  - `scope=empresa nfe nfse`    **Exemplo de corpo**  - `grant_type=client_credentials&client_id=SEU_CLIENT_ID&client_secret=SEU_CLIENT_SECRET&scope=empresa%20nfe`    **Uso do token nas requisições**  - `Authorization: Bearer SEU_ACCESS_TOKEN`
 *
 * The version of the OpenAPI document: 3.1.7
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
/**
 * Valor monetário total aproximado dos tributos, em conformidade com o artigo 1o da Lei no 12.741/2012.
 * @export
 * @interface TribTotalMonet
 */
export interface TribTotalMonet {
    /**
     * Valor monetário total aproximado dos tributos federais (R$).
     * @type {number}
     * @memberof TribTotalMonet
     */
    vTotTribFed: number | null;
    /**
     * Valor monetário total aproximado dos tributos estaduais (R$).
     * @type {number}
     * @memberof TribTotalMonet
     */
    vTotTribEst: number | null;
    /**
     * Valor monetário total aproximado dos tributos municipais (R$).
     * @type {number}
     * @memberof TribTotalMonet
     */
    vTotTribMun: number | null;
}

/**
 * Check if a given object implements the TribTotalMonet interface.
 */
export function instanceOfTribTotalMonet(value: object): boolean {
    let isInstance = true;
    isInstance = isInstance && "vTotTribFed" in value;
    isInstance = isInstance && "vTotTribEst" in value;
    isInstance = isInstance && "vTotTribMun" in value;

    return isInstance;
}

export function TribTotalMonetFromJSON(json: any): TribTotalMonet {
    return TribTotalMonetFromJSONTyped(json, false);
}

export function TribTotalMonetFromJSONTyped(json: any, ignoreDiscriminator: boolean): TribTotalMonet {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'vTotTribFed': json['vTotTribFed'],
        'vTotTribEst': json['vTotTribEst'],
        'vTotTribMun': json['vTotTribMun'],
    };
}

export function TribTotalMonetToJSON(value?: TribTotalMonet | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'vTotTribFed': value.vTotTribFed,
        'vTotTribEst': value.vTotTribEst,
        'vTotTribMun': value.vTotTribMun,
    };
}

