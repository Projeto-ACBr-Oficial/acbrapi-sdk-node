/* tslint:disable */
/* eslint-disable */
/**
 * ACBr API
 * API para automação comercial e documentos fiscais.    **Autenticação**  Esta API usa OAuth 2.0 no fluxo `client_credentials`.    **Ambientes disponíveis**  - Produção: `https://prod.acbr.api.br`  - Homologação: `https://hom.acbr.api.br`  - Autenticação para ambos os ambientes: `https://auth.acbr.api.br/realms/ACBrAPI/protocol/openid-connect/token`    **Endpoint para obter o token**  - `POST https://auth.acbr.api.br/realms/ACBrAPI/protocol/openid-connect/token`    **Content-Type**  - `application/x-www-form-urlencoded`    **Parâmetros do corpo**  - `grant_type=client_credentials`  - `client_id=SEU_CLIENT_ID`  - `client_secret=SEU_CLIENT_SECRET`  - `scope=empresa nfe nfse`    **Exemplo de corpo**  - `grant_type=client_credentials&client_id=SEU_CLIENT_ID&client_secret=SEU_CLIENT_SECRET&scope=empresa%20nfe`    **Uso do token nas requisições**  - `Authorization: Bearer SEU_ACCESS_TOKEN`
 *
 * The version of the OpenAPI document: 3.1.7
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
/**
 * Identificação do Fisco (uso exclusivo fisco).
 * @export
 * @interface DceSefazFisco
 */
export interface DceSefazFisco {
    /**
     * Número do CNPJ do órgão.
     * @type {string}
     * @memberof DceSefazFisco
     */
    CNPJ: string | null;
    /**
     * Nome do órgão.
     * @type {string}
     * @memberof DceSefazFisco
     */
    xOrgao: string | null;
    /**
     * Sigla da UF.
     * @type {string}
     * @memberof DceSefazFisco
     */
    UF: string | null;
}

/**
 * Check if a given object implements the DceSefazFisco interface.
 */
export function instanceOfDceSefazFisco(value: object): boolean {
    let isInstance = true;
    isInstance = isInstance && "CNPJ" in value;
    isInstance = isInstance && "xOrgao" in value;
    isInstance = isInstance && "UF" in value;

    return isInstance;
}

export function DceSefazFiscoFromJSON(json: any): DceSefazFisco {
    return DceSefazFiscoFromJSONTyped(json, false);
}

export function DceSefazFiscoFromJSONTyped(json: any, ignoreDiscriminator: boolean): DceSefazFisco {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'CNPJ': json['CNPJ'],
        'xOrgao': json['xOrgao'],
        'UF': json['UF'],
    };
}

export function DceSefazFiscoToJSON(value?: DceSefazFisco | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'CNPJ': value.CNPJ,
        'xOrgao': value.xOrgao,
        'UF': value.UF,
    };
}

