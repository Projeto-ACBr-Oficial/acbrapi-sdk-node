/* tslint:disable */
/* eslint-disable */
/**
 * ACBr API
 * API para automação comercial e documentos fiscais.    **Autenticação**  Esta API usa OAuth 2.0 no fluxo `client_credentials`.    **Ambientes disponíveis**  - Produção: `https://prod.acbr.api.br`  - Homologação: `https://hom.acbr.api.br`  - Autenticação para ambos os ambientes: `https://auth.acbr.api.br/realms/ACBrAPI/protocol/openid-connect/token`    **Endpoint para obter o token**  - `POST https://auth.acbr.api.br/realms/ACBrAPI/protocol/openid-connect/token`    **Content-Type**  - `application/x-www-form-urlencoded`    **Parâmetros do corpo**  - `grant_type=client_credentials`  - `client_id=SEU_CLIENT_ID`  - `client_secret=SEU_CLIENT_SECRET`  - `scope=empresa nfe nfse`    **Exemplo de corpo**  - `grant_type=client_credentials&client_id=SEU_CLIENT_ID&client_secret=SEU_CLIENT_SECRET&scope=empresa%20nfe`    **Uso do token nas requisições**  - `Authorization: Bearer SEU_ACCESS_TOKEN`
 *
 * The version of the OpenAPI document: 3.1.7
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
import type { DceSefazEndeDest } from './DceSefazEndeDest';
import {
    DceSefazEndeDestFromJSON,
    DceSefazEndeDestFromJSONTyped,
    DceSefazEndeDestToJSON,
} from './DceSefazEndeDest';

/**
 * Identificação do destinatário da DCe.
 * @export
 * @interface DceSefazDest
 */
export interface DceSefazDest {
    /**
     * Número do CNPJ.
     * Em caso de empresa não estabelecida no Brasil, será informado o CNPJ com zeros.
     * Informar os zeros não significativos.
     * @type {string}
     * @memberof DceSefazDest
     */
    CNPJ?: string | null;
    /**
     * Número do CPF.
     * Informar os zeros não significativos.
     * @type {string}
     * @memberof DceSefazDest
     */
    CPF?: string | null;
    /**
     * Identificação do destinatário outros.
     * Identificação do destinatário não obrigado a inscrição do CPF tais como estrangeiro, indígena e quilombola
     * Em caso de não contar CPF do assinante, informar o RG.
     * @type {string}
     * @memberof DceSefazDest
     */
    idOutros?: string | null;
    /**
     * Razão social ou Nome do destinatário.
     * @type {string}
     * @memberof DceSefazDest
     */
    xNome: string | null;
    /**
     * 
     * @type {DceSefazEndeDest}
     * @memberof DceSefazDest
     */
    enderDest: DceSefazEndeDest;
}

/**
 * Check if a given object implements the DceSefazDest interface.
 */
export function instanceOfDceSefazDest(value: object): boolean {
    let isInstance = true;
    isInstance = isInstance && "xNome" in value;
    isInstance = isInstance && "enderDest" in value;

    return isInstance;
}

export function DceSefazDestFromJSON(json: any): DceSefazDest {
    return DceSefazDestFromJSONTyped(json, false);
}

export function DceSefazDestFromJSONTyped(json: any, ignoreDiscriminator: boolean): DceSefazDest {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'CNPJ': !exists(json, 'CNPJ') ? undefined : json['CNPJ'],
        'CPF': !exists(json, 'CPF') ? undefined : json['CPF'],
        'idOutros': !exists(json, 'idOutros') ? undefined : json['idOutros'],
        'xNome': json['xNome'],
        'enderDest': DceSefazEndeDestFromJSON(json['enderDest']),
    };
}

export function DceSefazDestToJSON(value?: DceSefazDest | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'CNPJ': value.CNPJ,
        'CPF': value.CPF,
        'idOutros': value.idOutros,
        'xNome': value.xNome,
        'enderDest': DceSefazEndeDestToJSON(value.enderDest),
    };
}

